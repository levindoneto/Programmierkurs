...
class ProbabilisticLemmatizer:
    ...
    Implement these methods by filling in the blanks (i.e. 'fill()').
    ...
    """
    def __init__(self, token_lemma_counts):

        token2lemma2count= {}
        token2count = {}
        for t, l, c in token_lemma_counts:
            lemma2count = token2lemma2count.get(t, {})
            lemma2count[l] = lemma2count.get(l, 0) + c
            token2count[t] = token2count.get(t, 0) + c
    ...
    def get_prob_lemma(self, token, lemma):
        prob = self.token2lemma2prob.get(token, {}).get(lemma, 0)
        return prob
